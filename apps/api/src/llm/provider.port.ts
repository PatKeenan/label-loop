import type { JsonValue, JudgeOutput, ModelPin } from '@labelloop/contracts'
 ...
 * an exception list.
 *
 * The port is deliberately one method. A judge is one binary question about one output
...
export type JudgeCall = {
  ...
  question: string
  /**
   * The roles of the evaluation, in the caller's own shapes (ADR-0073). `output` is the ONE
   * thing judged — their agent's final answer or proposal, never generated by us (ADR-0019).
   * `input` is what led to it: evidence for the judgement, not on trial itself. `reference`
   * is the per-call facts needed to decide. `metadata` is deliberately absent — it is
   * bookkeeping, and a judge that could read it could be swayed by it.
   *
   * Adapters present these through `renderForModel`, never by formatting their own.
   */
  input: JsonValue
  output: JsonValue
