...
## Decision
The review endpoints return an annotator **only the trace pair** (artifact and context) and progress. No verdict, score, confidence, model, cost, key or trace id is in the response at all, so the annotator surface cannot leak what it never receives. Tests assert the payload's keys.
