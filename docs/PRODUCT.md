...
- **Annotation runs against an assigned set, never the whole panel.** A developer curates an **annotation set** — a named, snapshotted selection of a panel's traces, picked manually or by latest / earliest / random N — and assigns one or more annotators to it. Everyone assigned reviews the whole set, so overlaps are recorded rather than configured; where two answers differ, the **dictator** — one assigned annotator the developer names when there is more than one — is the one that counts. That is a rule for reading the rows, not a workflow: nothing is arbitrated by hand and both answers stay on the table. **An annotator has nothing to do until a set is assigned to them.** Staff read those answers on their own console surface and cannot edit them — nobody annotates somebody else's work (ADR-0079…0084).
