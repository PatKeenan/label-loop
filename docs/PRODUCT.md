...
5. **Annotate (open coding)** — SMEs annotate traces in a surface built for them: agree/correct each verdict, add free-text failure notes. Deliberately taxonomy-blind on the first pass, because showing categories up front anchors the expert and caps the taxonomy at whatever was already imagined.
...
- **Annotation runs against an assigned set, never the whole panel.** A developer curates an **annotation set** — a named, snapshotted selection of a panel's traces, picked manually or by latest / earliest / random N — and assigns one or more annotators to it. Everyone assigned annotates the whole set, so overlaps are recorded rather than configured; where two answers differ, the **dictator** — one assigned annotator the developer names when there is more than one — is the one that counts. That is a rule for reading the rows, not a workflow: nothing is arbitrated by hand and both answers stay on the table. **An annotator has nothing to do until a set is assigned to them.** Staff read those answers on their own console surface and cannot edit them — nobody annotates somebody else's work (ADR-0079…0084).
...
- **Versioning as evidence:** prompts, panels, judges, taxonomies, datasets, and models are versioned such that any past decision can be reconstructed: which model ran, what input it received, what it returned, who annotated it.
